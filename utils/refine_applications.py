...
def main():
    ...
    print(f"Found {len(bridge2ai_standards)} standards with used_in_bridge2ai=true\n")
    ...
    for std in bridge2ai_standards:
        ...
        for app in std['has_application']:
            ...
            if remove_redundant_urls(app, parent_urls):
                ...
                changes_made.append(f"{std_id} / {app_id}: Removed redundant URLs")
            ...
            if 'references' in app and app['references']:
                ...
                for ref in app['references']:
                    ...
                    if not is_valid:
                        print(f"      ❌ BROKEN")
                        changes_made.append(f"{std_id} / {app_id}: Broken URL: {ref}")
                    else:
                        print(f"      ✓ OK")
            ...
            if 'references' not in app or len(app['references']) < 2:
                print(f"  ⚠️  Only {len(app.get('references', []))} references - needs more")
                changes_made.append(f"{std_id} / {app_id}: Insufficient references ({len(app.get('references', []))})")
