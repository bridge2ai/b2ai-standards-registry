...
def convert_substrate_links(substrate_list: list, all_data: Dict[str, Dict], relative_path_prefix: str = "..") -> list:
    ...
    for substrate in substrate_list:
        ...
        if substrate_str in all_data:
            ...
            slug = slugify(name)
            converted.append(f"[{substrate_str}]({relative_path_prefix}/substrates/{slug}.markdown) ({name})")
        else:
            # Try to convert any embedded IDs in the string
            converted.append(convert_ids_to_links(substrate_str, all_data, relative_path_prefix))
    return converted
...
def convert_topic_links(topic_list: list, all_data: Dict[str, Dict], relative_path_prefix: str = "..") -> list:
    ...
    for topic in topic_list:
        ...
        if topic_str in all_data:
            ...
            # Topics use the exact name with spaces removed but capitalization preserved
            slug = name.replace(' ', '')
            converted.append(f"[{topic_str}]({relative_path_prefix}/topics/{slug}.markdown) ({name})")
        else:
            # Try to convert any embedded IDs in the string
            converted.append(convert_ids_to_links(topic_str, all_data, relative_path_prefix))
    return converted